...
*Примечание: для событий mouseover, mouseout нельзя доверять свойству event.type — там может оказаться mousemove. Так происходит, поскольку событие на элементе внутри канваса не настоящее браузерное, а прокидывается из события mousemove.*
