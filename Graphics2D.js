...
//			}
	...
	Path = Class(Shape, {
		...
		transformPoints : function(a,b,c,d,e,f, px,py){
			function twoargs(){
				px = d;
				py = e;
			...
			var funcs = {
				...
				'skew' : function(){
					twoargs();
					a = d = 1;
					b = Math.tan(angle([c,c = Math.tan(angle(b))][0]));
					e = f = 0;
				}
			};
			if(a in funcs)
			...
			var pivot  = corner(py != null ? [px,py] : px == null ? 'center' : px, this.bounds()),
				matrix = [a,b,c,d,e,f],
				points = this.points();
			matrix = concat([ 1,0,0,1, pivot[0], pivot[1] ], matrix);
			matrix = concat(matrix, [ 1,0,0,1, -pivot[0], -pivot[1] ]);

			points.forEach(function(point,i){
				var x = parseInt(point.arg[0]),
					y = parseInt(point.arg[1]);

				point.arg[0] = x * matrix[0] + y * matrix[1] + matrix[4];
				point.arg[1] = x * matrix[2] + y * matrix[3] + matrix[5];
			});
			this.points(points);
